...
/*
    ...
    Additional members for Repeating:

        uint32_t    held // number of microseconds button was held ON

        Repeating strategy is related to the Timed strategy, and you should run that example first.
        
        Repeating counts how long it is held in the ON position. While it is held ON, it continues
        to callback (at a user-defined rate). On each call (including the final OFF transition) it has the 
        number of microseconds the button was held ON for in the extra variable 'held'
...
#define U_DBTIME_MS    2
#define U_FREQUENCY    1000

void h4setup() { // H4 constructor starts Serial
    Serial.println("H4P_GPIOManager Repeating Example v"H4P_VERSION);
    ...
    h4gm.Repeating(USER_BTN,INPUT,UB_ACTIVE,U_DBTIME_MS,U_FREQUENCY,[](H4GPIOPin* ptr){
        H4GM_PIN(Repeating); // Create the correct pointer type in 'pin'  
        h4gm.logicalWrite(LED_BUILTIN,pin->state); 
        if(pin->state) {
        if(pin->held) {
            Serial.print("Still held after ");Serial.print(pin->held);Serial.println(" microseconds"); // low-level pin times are uS
        } else Serial.println("START TIMING");
        }
        else Serial.print("LED WAS ON FOR : ");Serial.print(pin->held);Serial.println(" microseconds"); // low-level pin times are uS
    });
}
