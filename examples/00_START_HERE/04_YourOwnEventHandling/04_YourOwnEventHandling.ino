...
// event listeners
H4P_EventListener allexceptmsg(H4PE_ALMOST_ALL &~ H4PE_MSG,[](const string& svc,H4PE_TYPE t,const string& msg){ 
  Serial.printf("Usr2 sez %s %s\n",CSTR(h4pGetEventName(t)),CSTR(msg)); 
});
H4P_EventListener hb(H4PE_HEARTBEAT,[](const string& svc,H4PE_TYPE t,const string& msg){ 
...
void h4setup(){
  ...
  h4pOnEvent(H4PE_MSG,[](const string& msg){ Serial.printf("User message event occurred %s\n",CSTR(msg)); });
}