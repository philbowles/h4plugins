...
//**************************************************************************************************************
smartClient::smartClient(const string& fullurl):
    ...
    //, _headers(nullptr)
{
    Serial.printf("Smart client CTOR %s\n",CSTR(fullurl));
    _parseURL(fullurl);
    _connect();
}
...
bool  smartClient::_parseURL(const string& url){
    if(url.find("http",0)) return _parseURL(string(httpTag())+url);
