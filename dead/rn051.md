...
* Additional functions in H4P_VerboseMessages
...
* `h4vm.getTaskType` translates numeric task types into human-readable form
* `h4vm.getTaskName` translates numeric task names into human-readable form
...
[H4P_AsyncWebServer](h4wifi.md) no longer provides admin username/password, hence they are removed from its constructor. 
...
Stopping the `cmd` service improves performance by up to 50% (400% on STM32-NUCLEO!) so should be considered for all production code. The downside is that you are no longer able to issue Serial console commands. This can be reversed by remote restart of the cmd service (e.g. MQTT, HTTP REST, the new Web Console).

The default is false for development and testing, i.e do NOT auto-stop the `cmd` service (and therefore *do* allow Serial console commands) at the expense of overall performance.

Finally, the "handle" to the SerialCmd service has been changed from `h4sc` to `h4cmd` bit its service name remains the same at `cmd`
...
All refernces, functions, commands related to "trusted names" have been removed, as the same functionality is now provided by service names an the human-readable values for these is now available in the [new functions](#functions) `h4vm.getTaskType` and `h4vm.getTaskName`
